        ...
        switch (toolTipMode) {
            case AUTOMATIC:
                ...
                FacebookSdk.getExecutor().execute(new Runnable() {
                    @Override
                    public void run() {
                        FetchedAppSettings settings=null;
                        if(appId!=null)
                         settings = Utility.queryAppSettings(appId, false);
                        final FetchedAppSettings finalSettings = settings;
                        getActivity().runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                showToolTipPerSettings(finalSettings);
                            }
                        });
                    }
                });
